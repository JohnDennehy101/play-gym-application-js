...
*/
  register(request, response) {
    ...
    if (
      ...
      (user.gender.charAt(0).toLowerCase() === "m" || user.gender.charAt(0).toLowerCase() === "f") &&
      user.email.length > 1 &&
      ...
      user.height > 0.1 &&
      user.height <= 2.2 &&
      user.startingWeight >= 1 &&
      user.startingWeight <= 300
    ) {
      user.id = uuid.v1();
    ...
    if (
      user.gender.length < 1 ||
      user.gender.charAt(0).toLowerCase() !== "m" ||
      user.gender.charAt(0).toLowerCase() !== "f"
    ) {
      invalidGender = true;
    }
